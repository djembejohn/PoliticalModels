...
def main():
	...
	while current_round <= ROUNDS and len(agents) > 2:
		...
		awakenAgents()
		food = getFood()*MAX_AGENTS/float(len(agents))
		...
		# Instead just shuffle the list and go through 2 at a time
		shuffle(agents)
...
def compete(agent1, agent2, food):
#	winner = choice([agent, nemesis])
#	agent2 = agent if (agent1 is nemesis) else nemesis

	if agent1.agent_type == TYPE_HAWK and agent2.agent_type == TYPE_HAWK:
		# Random agent1 chosen, agent2 gets injured, agent1 gets food
		agent1.energy += getEnergyFromFood(food)/2		 
		agent1.energy  -= ENERGY_LOSS_FROM_FIGHTING
		agent2.energy += getEnergyFromFood(food)/2		 
		agent2.energy  -= ENERGY_LOSS_FROM_FIGHTING

	if agent1.agent_type == TYPE_HAWK and agent2.agent_type == TYPE_DOVE:
		agent1.energy += getEnergyFromFood(food)
		agent2.energy -= ENERGY_COST_OF_BLUFFING

	if agent1.agent_type == TYPE_DOVE and agent2.agent_type == TYPE_HAWK:
		agent2.energy += getEnergyFromFood(food)
		agent1.energy -= ENERGY_COST_OF_BLUFFING

	if agent1.agent_type == TYPE_DOVE and agent2.agent_type == TYPE_DOVE:
		agent1.energy += getEnergyFromFood(food)/2
		agent1.energy  -= ENERGY_COST_OF_BLUFFING
		agent2.energy += getEnergyFromFood(food)/2
		agent2.energy  -= ENERGY_COST_OF_BLUFFING

	agent2.status = agent1.status = STATUS_ASLEEP
